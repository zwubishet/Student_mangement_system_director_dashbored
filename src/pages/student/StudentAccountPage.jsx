...
export default function StudentAccountPage() {
  ...
  const changePassword = async (e) => {
    ...
    setSaving(true);
    try {
      await studentPortalApi.changePassword({
    ...
    <StudentLayout>
      ...
      {profile && (
        <div className={`${ui.card} p-5 mb-6 flex items-start gap-4`}>
          <div className="w-12 h-12 rounded-2xl bg-sky-100 dark:bg-sky-950/50 flex items-center justify-center">
          ...
          <div>
            ...
            <p className={`text-sm ${ui.muted}`}>
              {profile.admission_number} · {profile.grade_name || '—'} · {profile.section_name || '—'}
            </p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={changePassword} className={`${ui.card} p-6 max-w-md space-y-4`}>
        <p className={`font-black flex items-center gap-2 ${ui.panelTitle}`}>
          <KeyRound size={18} className="text-sky-600" /> Change password
        </p>
        <Input
