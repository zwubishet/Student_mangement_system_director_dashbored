...
export default function TeacherStudentDetailPage() {
  ...
  if (loading || !student) {
    return <TeacherLayout><div className="h-48 bg-white rounded-2xl animate-pulse" /></TeacherLayout>;
  }
  ...
  return (
    <TeacherLayout title={`${student.first_name} ${student.last_name}`} subtitle={student.admission_number}>
      <div className="space-y-6 max-w-3xl">
        ...
        <section className="bg-white border rounded-2xl p-6 space-y-3 text-sm">
        ...
        <section className="bg-white border rounded-2xl p-6">
          ...
          <ul className="space-y-2">
            {student.enrollments?.map((e) => (
              <li key={e.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
