...
import { CheckCircle2, Loader2, Receipt, RefreshCw, Users, XCircle } from 'lucide-react';
...
export default function FinanceApprovalsPanel() {
  ...
  const [data, setData] = useState({ payroll: [], fee_requests: [] });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(unwrap(res) || { payroll: [], fee_requests: [] });
    } catch (e) {
  ...
  const pendingCount = (data.payroll?.length || 0) + (data.fee_requests?.length || 0);
